...
def add_to_cart(request, product_id):
    ...
    # Проверка на доступное количество товара
    if cart_item.quantity < product.quantity:
        cart_item.quantity += 1
        cart_item.save()
    else:
        # Если количество превышает доступное, возвращаем ошибку
...
def update_cart_item_quantity(request, item_id, action):
    ...
    if action == 'increase':
        # Проверка, чтобы количество не превышало доступное на складе
        if cart_item.quantity < product.quantity:
            cart_item.quantity += 1
            cart_item.save()
        else:
            return JsonResponse({'quantity': cart_item.quantity}, status=400)
